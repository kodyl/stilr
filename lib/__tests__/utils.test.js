...
const hyphenateStyleName = (styleName) => {
  return styleName
...
const createMarkup = (styleDeclaration) => {
  const rules = Object.keys(styleDeclaration).map((ruleName) => {
    const ruleValue = styleDeclaration[ruleName];
    const hyphenatedStyleName = hyphenateStyleName(ruleName);

    return `${hyphenatedStyleName}:${ruleValue};`;
  });
  return rules.join('');
};
...
describe('utils', () => {
  describe('#hyphenateStyleName', () => {
    it('hyphanates fontSize', () => {
      const hyphenatedStyleName = hyphenateStyleName('fontSize');
      assert.equal(hyphenatedStyleName, 'font-size');
    });

    it('doesn\'t hyphenates opacity', () => {
      const hyphenatedStyleName = hyphenateStyleName('opacity');
      assert.equal(hyphenatedStyleName, 'opacity');
    });

    it('doesn\'t hyphenates already hyphanated style name', () => {
      const hyphenatedStyleName = hyphenateStyleName('font-size');
      assert.equal(hyphenatedStyleName, 'font-size');
    });

    it('supports vendor prefixes', () => {
      const hyphenatedStyleName = hyphenateStyleName('-moz-opacity');
  ...
  describe('#createMarkup', () => {
    ...
    it('creates hyphenate style names', () => {
      const markup = createMarkup({fontSize: '10px'});
      assert.equal(markup, 'font-size:10px;')
    });

    xit('converts numbers to px values', () => {
      const markup = createMarkup({fontSize: 10});
      assert.equal(markup, 'font-size:10px;');
    });

    xit('creates no markup for empty object', () => {
      const markup = createMarkup({});
      // TODO: current behaviour discuss,
      // if we should just return an empty string
