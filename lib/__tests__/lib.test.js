...
describe('stilr', () => {
  ...
  describe('#create(obj)', () => {
    ...
    it('creates an entry in the stylesheet', () => {
      ...
      assert.equal(StyleSheet.__stylesheet.size, entries.length);
    });
    ...
    it('has the same keys', () => {
      ...
      assert.equal(keys.join(''), originalKeys.join(''));
    });
    ...
    it('assigns the same class name to duplicate styles', () => {
      ...
      classNames.map(className =>
        assert.equal(styles[classNames[0]], styles[className])
      );
    });
    ...
    it('accepts a alternative stylesheet', () => {
      ...
      StyleSheet.create(style.basic);
      assert.equal(altStylesheet.size, entries.length);

      assert.notEqual(altStylesheet, StyleSheet.__stylesheet);
    });
    ...
    it('assigns unique class names', () => {
      ...
      assert.equal(keys.length, classes.length);
    });
  });
  ...
  describe('#render()', () => {
    ...
    it('is parseable css', () => {
      ...
      const renderedClassNames = parsedCss.stylesheet.rules.map(
        rule => rule.selectors[0]
      );

      for (const className of StyleSheet.__stylesheet.keys()) {
        assert.ok(
          renderedClassNames.indexOf(`.${className}`) !== -1,
          `.${className} should be part of the rendered css classes: '${renderedClassNames}'`
        );
      }
    });
    ...
    it('outputs pseudo classes correctly', () => {
      ...
      assert.ok(css.match(hover).length === 1);
    });
    ...
    it('outputs media queries correctly', () => {
      ...
      assert.equal(
        rules[0].type,
        'rule',
        'Rule should be before media queries'
      );
    ...
    it('should not output empty styles', () => {
      ...
      assert.equal(
        parsedCss.stylesheet.rules.length,
        2,
        'stylesheet should contain two rules'
      );
      assert.equal(
        parsedCss.stylesheet.rules[1].rules.length,
        1,
    ...
    it('handles nested pseudo selectors in media queries', () => {
      ...
      assert.equal(
        rules[2].declarations[0].value,
        styles.base[':active'].color
      );
      ...
      assert.equal(
        mediaRules[0].declarations[0].value,
        styles.base[breakpoint].color
      );
      assert.equal(
        mediaRules[1].declarations[0].value,
        styles.base[breakpoint][':hover'].color
      );
      assert.equal(
        mediaRules[2].declarations[0].value,
        styles.base[breakpoint][':active'].color
      );
    });
    ...
    it('handles values as arrays', () => {
      const styles = {
        base: {
          boxShadow: ['1px 1px black', '0 0 5px red']
        }
      };
      StyleSheet.create(styles);
